...
public final class DatabaseManager {
    ...
    public static void initializeSchema() {
        try (Connection conn = getConnection();
            ...
            // Ensure unit_cost column exists on existing databases (historical
            // cost-basis snapshot, so COGS/profit reports don't shift when a
            // product's current unit_cost is edited later)
            try {
                st.execute("ALTER TABLE stock_movement ADD COLUMN unit_cost REAL NOT NULL DEFAULT 0;");
            } catch (SQLException ignored) {
                // Column already exists
            }
